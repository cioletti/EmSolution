...
public class ReadAlertaPlJob implements Job {
	...
	String bookMark = "https://www.myvisionlink.com/APIService/CATDataTopics/v5/feed/ParanaEquip/FenceAlert/";
	...
	public void execute(JobExecutionContext arg0) throws JobExecutionException {
		...
		//while(isConnection){
			try {
				...
				Query query = manager.createQuery("From EmsAlertaBuffer");
				EmsAlertaBuffer fuelBuffer = new EmsAlertaBuffer();
				fuelBuffer.setBuffer(0L);
				if(query.getResultList().size() > 0){
					fuelBuffer = (EmsAlertaBuffer)query.getResultList().get(0);
				}
				...
				EmailHelper emailHelper = new EmailHelper();
				emailHelper.sendSimpleMail("Erro ao executar serviço de importação de alarmes do Product Link ALERTA: "+e1.getMessage(), "ERRO ALERTA", "rodrigo@rdrsistemas.com.br");
			}
			try {
				manager = JpaUtil.getInstance();
