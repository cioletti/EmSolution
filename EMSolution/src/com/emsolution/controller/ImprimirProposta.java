...
/**
 * Servlet implementation class ImprimirProposta
 */
public class ImprimirProposta extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ImprimirProposta() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try{
			...
			DiagnosticoBean diagnosticoBean = diagnosticoBusiness.findallPipPspGarantia(detalhesProposta.getIdProposta().getNumSerie().toUpperCase());
			...
			query = manager.createNativeQuery("select PERC_LEVEL_UTILIZATION from EMS_FUEL where RECEIVED_TIME = ("+
					"	select MAX(RECEIVED_TIME) from EMS_FUEL where SERIAL_NUMBER = '"+detalhesProposta.getIdProposta().getNumSerie().toUpperCase()+"')"+
					"	and SERIAL_NUMBER = '"+detalhesProposta.getIdProposta().getNumSerie().toUpperCase()+"'");
			BigDecimal PERC_LEVEL_UTILIZATION = BigDecimal.ZERO;
			if(query.getResultList().size() > 0){
				PERC_LEVEL_UTILIZATION = (BigDecimal)query.getResultList().get(0);
				if(PERC_LEVEL_UTILIZATION == null){
					PERC_LEVEL_UTILIZATION = BigDecimal.ZERO;
				}
			}
...
//			}
			...
			if(detalhesProposta.getIdProposta().getNumSerie().length() >= 4){
				parametros.put("PREFIXO", detalhesProposta.getIdProposta().getNumSerie().toUpperCase().subSequence(0, 4));
			}else{
				parametros.put("PREFIXO", detalhesProposta.getIdProposta().getNumSerie().toUpperCase().subSequence(0, 4));
			}
			...
			if(detalhesProposta.getIdProposta().getTipoCliente() != null && !detalhesProposta.getIdProposta().getTipoCliente().equals("INT")){
				SQL = "select distinct ((pre.valor_de_venda * ch.fator_cliente)* "+
						...
						" THEN (select fator_urgencia from sc_fator) ELSE 1 END) * seg.qtd_comp valor_total_hora, seg.horas_prevista,"+
